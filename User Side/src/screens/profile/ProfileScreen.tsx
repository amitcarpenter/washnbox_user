...
const ProfileScreen = () => {
  ...
  const number = useSelector(state=>state.loginData.number)
  ...
  const renderInputFields = () => {
    const { countryCode, localNumber } = extractPhoneDetails(profile?.mobile_number || "");
    return (
      <View>
        <Input 
          editable={false}
  ...
  return (
    <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={styles.scrollViewContainer}>
      ...
      {renderProfileImage()}
      {renderInputFields()}
      <Button disabled={true} onPress={sumitDetails} title='Update' style={styles.updateButton} />
    </ScrollView>
  );
};
