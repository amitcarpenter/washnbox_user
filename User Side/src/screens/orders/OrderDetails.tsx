...
import { addClothType, removeClothType, updateService, updateQuantity } from '../../redux/dataSlice';
import CustomDropdown from '../../component/custom_dropdown/CustomDropdown'
import { getUserToken, makeGetApiCall, makePostApiCall } from '../../utils/helper'
...
const OrderDetails = () => {
  ...
  const  [venderServices, setVenderServices] = useState(venderData?.all_services) 
  ...
  const prepareOrderData = () => {
    const formattedItems = selectedClothTypeItem.map(item => ({
      item_name: item.type, // Cloth type (e.g., "T-shirt", "Jeans")
      quantity: item.quantity, // Selected quantity
      service_type: item.selectedService, // Selected service (e.g., "Wash", "Iron")
      price_per_unit: venderData?.all_services.find(service => service.name === item.selectedService)?.price || 0
    }));
  
    // Calculate total price
    const totalPrice = formattedItems.reduce((acc, item) => acc + (item.quantity * item.price_per_unit), 0);
  
    const orderData = {
      order_id: 1, // Replace with dynamic ID if needed
  ...
  const submitOrder = async () =>{
    ...
    const url = PROVIDER_URLS.UPDATE_ORDER_DETAILS
    ...
    return(
      <View style={{width:"100%",minHeight:"85%"}}>
        ...
        <View style={styles.lockboxAndCallIconContainer}>
          <Text style={styles.lockboxText}>{  timeSlot  || "Provide lock box drop off"}</Text>
          <TouchableOpacity onPress={openTimeSlotBottomSheet} style={styles.callIconButton}>
  ...
  const onSelectClothType = (item) => {
    if (selectedClothId.includes(item.id)) {
      ...
      let filteredSelectedIds = selectedClothId.filter(id => id !== item.id);
      let filteredSelectedClothTypeItems = selectedClothTypeItem.filter(selected => selected.id !== item.id);
      setSelectedClothId([...filteredSelectedIds]);
      ...
      const servicePrice = venderData?.all_services.find(service => service.name === defaultService)?.price || 0;
  ...
  const increaseCount = (id) => {
    setSelectedClothTypeItems((prevItems) =>
      prevItems.map((item) => {
        if (item.id === id) {
          const servicePrice = venderData?.all_services.find(service => service.name === item.selectedService)?.price || 0;
          return {
            ...item,
  ...
  return (
    <View>
        ...
        <View style={styles.bottomModalButtonContainer}>
          ...
          <Text style={{color:"black",fontSize:28,fontWeight:"700",marginLeft:30}}>Pay</Text>
          <Button 
            style={{width:"45%",alignSelf:"flex-end",right:30}} 
            titleStyle={{fontSize:28}}
            onPress={submitOrder}  title={`$ ${prepareOrderData().total_price}`}  />
        </View>
        {renderModal()}
        {renderBottomSheet()}
